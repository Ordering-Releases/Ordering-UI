...
export const HelmetTags = (props) => {
  ...
  return (
    <Helmet titleTemplate={!page ? '' : `${theme?.my_products?.components?.website_settings?.components?.values?.name || settings.app_name} - %s`}>
      ...
      <meta name='description' content={theme?.my_products?.components?.website_settings?.components?.values?.description || props.helmetMetaTags?.description || metaTag.description} />
