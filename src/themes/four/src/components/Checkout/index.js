...
const CheckoutUI = (props) => {
  ...
  const [isDriverTipOpen, setIsDriverTipOpen] = useState(false)
  ...
  const toggleAccordion = (type) => {
    setActiveState({
      ...setActive,
      [type]: setActive[type] === '' ? 'active' : ''
    })
    setRotateState({
      ...setRotate,
      [type]: setActive[type] === 'active' ? 'accordion__icon' : 'accordion__icon rotate'
    })
  }
        ...
        <WrappContainer>
          <LeftContainer>
            ...
            {!props.isHideSectionThree && (
              <BusinessDetailsContainer>
                ...
                {!cartState.loading && businessDetails?.business && Object.values(businessDetails?.business)?.length > 0 && (
                  <>
                    <Accordion
                      className={`accordion ${setActive?.business}`}
                      onClick={() => toggleAccordion('business')}
                    >
                      <h1><BiStoreAlt /> {t('BUSINESS_DETAILS', 'Business Details')}</h1>
                      <span>
                        <IosArrowDown className={`${setRotate.business}`} />
                      </span>
                    </Accordion>
                    <AccordionContent
                      style={{ maxHeight: !setActive?.business && '0px' }}
                    >
            ...
            {!props.isHideSectionFour && !cartState.loading && cart && (
              <PaymentMethodContainer>
                <Accordion
                  className={`accordion ${setActive.payment}`}
                  onClick={() => toggleAccordion('payment')}
                >
                  <h1><BsCreditCard /> {t('PAYMENT_METHODS', 'Payment Methods')}</h1>
                  <span>
                    <IosArrowDown className={`${setRotate.payment}`} />
                  </span>
                </Accordion>
                <AccordionContent
                  style={{ maxHeight: !setActive.payment && '0px' }}
                >
          ...
          <RightContainer>
            <RightInnerContainer>
              ...
              {!props.isHideSectionSix && !cartState.loading && cart && cart?.status !== 2 && (
                <WrapperPlaceOrderButton>
                  <Button
              ...
              {cart?.valid_products && (
                <OrderBill>
                  ...
                  {!cartState.loading &&
                    ...
                    (
                      <table className='driverTip'>
                        <tbody>
                          <tr>
                            <td>
                              <span>{t('DRIVER_TIP', 'Driver tip')} </span>
                              <Button
                                color='primary'
                                outline
                                onClick={() => setIsDriverTipOpen(true)}
                              >
                                {t('CHANGE', 'Change')}
                              </Button>
                          ...
                          <tr>
                            <td colSpan={2}>
                              <div className='recommendTxt'>
                                {t('RECOMMENDATION_DRIVER_TIP', 'Want to recognize your delivery person’s efforts? Consider a larger tip as a thank you -100% of the tip goes to them.')}
                              </div>
                            </td>
                          </tr>
                        </tbody>
              ...
              >
                <DriverTipContainer>
                  <h1>{t('TIP_YOUR_DELIVERY_PERSON', 'Tip your delivery person?')}</h1>
                  <ul>
                    <li>{t('CUSTOM_DRIVER_TIP_MESSAGE', '100% of these tips go directly to your driver')}</li>
                    <li>{t('Tips are always optional')}</li>
                  </ul>
                  <DriverTips
                    businessId={cart?.business_id}
        ...
        <Container>
          <WrappContainer>
            <LeftContainer isSkeleton>
              ...
              <Skeleton height={35} count={3} style={{ marginBottom: '10px' }} />
