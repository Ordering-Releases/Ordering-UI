...
const PaymentOptionsUI = (props) => {
  ...
  const handlePaymentMethodClick = (paymethodId) => {
    const selectedPaymethod = paymethodsList.paymethods.find(paymethod => paymethod.id === paymethodId)
    const isPopupMethod = ['stripe', 'stripe_direct', 'stripe_connect', 'stripe_redirect', 'paypal'].includes(selectedPaymethod?.gateway)
  ...
  const placeholder = <PayCard>{t('SELECT_PAYMETHOD', 'Select a paymethod')}</PayCard>
  return (
    <>
        ...
        <PaymentMethodsList className='payments-list'>
          {paymethodsList.paymethods.length > 0 &&
            <Select
              ...
              onChange={(paymethod) => handlePaymentMethodClick(paymethod)}
              isFullWidth
            />}
