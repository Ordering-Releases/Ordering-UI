        ...
        <WrapRightContainer>
          ...
          <WrapRightInnerContainer>
            ...
            {!props.isHideSectionSeven && !cartState.loading && cart && cart?.status !== 2 && (
              <WrapperPlaceOrderButton>
                <Button
                  color={(!cart?.valid_maximum || (!cart?.valid_minimum && !(cart?.discount_type === 1 && cart?.discount_rate === 100))) ? 'secundary' : 'primaryGradient'}
...
export const Checkout = (props) => {
  ...
  return (
    <>
      <NavBar title={t('MOBILE_FRONT_CHECKOUT', 'Checkout')} />
      ...
      {cartState.loading && !(window.location.pathname === '/checkout') && (
        <Container>
          ...
          <WrapRightContainer>
            <WrapRightInnerContainer>
              <Skeleton height={100} style={{ marginBottom: '30px' }} />
              <Skeleton height={500} style={{ marginBottom: '30px' }} />
            </WrapRightInnerContainer>
          </WrapRightContainer>
        </Container>
      )}
